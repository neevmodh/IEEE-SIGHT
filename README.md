...
| F2 | **Inner Page Canvas** | Dedicated particle systems on Team, Events & Contact pages |
...
| F7 | **Counter Animations** | Scroll-triggered counting from 0 → target with easing |
| F8 | **Stats Banner Counter** | Animated team/event stats with formatted numbers |
...
| F30 | **Marquee Scroll** | Infinite horizontal text scroll with velocity sync |
...
│  │                                             │   │
│  │  • initPreloader()    • initHeroCanvas()    │   │
│  │  • initLenis()        • initSplitText()     │   │
│  │  • initNavbar()       • initCounters()      │   │
│  │  • initCursor()       • initMagnetic()      │   │
│  │  • initTilt()         • initTimeline()      │   │
│  │  • initCountdown()    • initMarquee()       │   │
│  │  • initFooterReveal() • 20+ more modules... │   │
│  └─────────────────────────────────────────────┘   │
│       │                                            │
│       ▼                                            │
│  ┌─────────────────────────────────────────────┐   │
...
│       ▼                                            │
│  ┌──────────┐ ┌──────────┐ ┌────────┐ ┌────────┐  │
│  │index.html│ │team.html │ │events  │ │contact │  │
│  │  (Home)  │ │  (Team)  │ │ .html  │ │ .html  │  │
│  └──────────┘ └──────────┘ └────────┘ └────────┘  │
│                                                     │
└─────────────────────────────────────────────────────┘
...
### 🏠 Home (`index.html`)
The flagship page — a full cinematic experience with:
...
- Mission & Vision cards with reveal animations
- "Idea → Impact" journey timeline with scroll-driven progress
...
- 14 team member cards with:
  - Real photos (grayscale → color on hover)
  - Role badges + skill tags
...
- Animated form with:
  - Focus-state label color transitions
  - Gradient underline on active inputs
...
| `--gradient` | `purple → pink` | Gradient elements |

### Typography

| Font | Weight | Usage |
|------|--------|-------|
| **Syne** | 400–800 | Display / Headlines |
| **Space Grotesk** | 300–700 | Body text |
| **Inter** | 300–500 | UI elements |
| `'SF Mono'` fallback | — | Monospace / labels |
...
git clone https://github.com/your-username/ieee-sight-sgnu.git
cd ieee-sight-sgnu
...
│                           #    30+ modular animation functions
...
│   │   └── vanshvi.jpg
│   │
│   └── images/                    # 🖼 Project & event assets
│       ├── hero-bg.png
│       ├── project-*.png          # 8 project illustrations
│       └── event-*.png            # 3 event illustrations
│
└── README.md               # 📖 This file
```
...
| **Scroll Sync** | Lenis → `ScrollTrigger.update()` via `requestAnimationFrame` |
...
| **Dhruv Patel** | Content Head | ✍️ Content & Marketing |
| **Gargee Patel** | Marketing Head | ✍️ Content & Marketing |
| **Poojan Parmar** | Social Media Head | ✍️ Content & Marketing |
